...
class AspectExtractor:
    """Извлекает аспекты из текста с помощью ML модели"""
    ...
    def extract_aspects(self, text: str) -> Tuple[List[str], List[str]]:
        """Извлекает позитивные и негативные аспекты из текста"""
        if not text.strip():
            return [], []
    ...
    def _get_model_predictions(self, text: str) -> Tuple[List[str], List[float], List[tuple]]:
        """Получает предсказания модели для текста"""
    ...
    def _process_bio_predictions(self, text: str, predictions: List[str], 
        ...
        """Обрабатывает BIO теги для извлечения аспектов"""
        positive_aspects = []
        negative_aspects = []
    ...
    def _extract_with_sliding_window(self, text: str) -> Tuple[List[str], List[str]]:
        """Извлечение аспектов с использованием скользящего окна"""
        all_positive = []
        all_negative = []

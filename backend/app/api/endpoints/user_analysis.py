...
async def process_analysis_background(analysis_id: int):
    ...
    async for db in get_async_session():
        try:
            ...
            # Проверяем, не выполнен ли уже live анализ
            existing_result = await crud_analysis.get_result(db, request_id=analysis_id)
            if existing_result:
                # Результаты уже есть (live анализ был выполнен), пропускаем фоновую обработку
            ...
            # Используем analyze_sentiment для получения категоризированных аспектов
            ...
            # Извлекаем категоризированные аспекты из analyze_sentiment
            ...
            # Извлекаем плоские списки аспектов для сохранения
            ...
            # Формируем структуру aspect_categories с правильными категориями
...
async def get_analysis_progress(
    ...
    try:
        ...
        # Оценка оставшегося времени на основе прогресса
